/*--
	Comandos basicos
		Estos comandos son extremadamente
		requeridos por el servidor y la mayoria
		son ejecutados y enviados por el cliente.

		Si se quiere agregar mas comandos o
		plugins se recomienda agregarlos en la
		carpeta 'plugins'.
--*/

// Para futuro: Poner chat.js y chat-commands.js en un solo archivo
...
exports.commands = {
	...
	hotpatch: function (target, room, user) {
		...
		try {
			if (target === 'chat' || target === 'commands') {
				...
				delete require.cache[require.resolve('./plugins/info')];
				global.Chat = require('./chat');

				Chat.uncacheTree('./plugins/utils');
				delete require.cache[require.resolve('./plugins/utils/index')];
				global.Utils = require('./plugins/utils');
	...
	savelearnsets: function (target, room, user) {
		if (!this.can('hotpatch')) return false;
		fs.writeFile('tiers/data/learnsets.js', 'exports.BattleLearnsets = ' + JSON.stringify(Tools.data.Learnsets) + ";\n");
...
process.nextTick(() => {
	// Querríamos migrar la mayoría de estos datos a un esquema JSON
