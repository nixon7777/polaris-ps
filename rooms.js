...
class GlobalRoom {
	constructor(roomid) {
		...
		this.chatRoomData = [];
		try {
			this.chatRoomData = require(CONFIG_DIR + 'chatrooms.json');
		...
		for (let i = 0; i < this.chatRoomData.length; i++) {
			...
			if (!Config.quietconsole) console.log("Sala ", id.magenta.bold + '... Sin errores');
		...
		this.writeChatRoomData = (() => {
			...
			return () => {
				...
				fs.writeFile(CONFIG_DIR + 'chatrooms.json.0', data, () => {
					data = null;
					fs.rename(CONFIG_DIR + 'chatrooms.json.0', CONFIG_DIR + 'chatrooms.json', () => {
						writing = false;
						if (writePending) {
							writePending = false;
...
class BattleRoom extends Room {
	...
	logBattle(p1score, p1rating, p2rating) {
		...
		let curpath = LOGS_DIR + logfolder;
...
class ChatRoom extends Room {
	constructor(roomid, title, options) {
		...
		if (Config.logchat) {
			...
			this.logEntry('Cargando sala: ' + this.id);
	...
	rollLogFile(sync) {
		...
		let date = new Date();
		let basepath = LOGS_DIR + 'chat/' + this.id + '/';
		mkdir(basepath, '0755', () => {
...
if (!Config.quietconsole) console.log("Obteniendo salas:".green.underline);
