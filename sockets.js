...
global.CONFIG_DIR = (process.env.OPENSHIFT_DATA_DIR) ? process.env.OPENSHIFT_DATA_DIR : './config/';
...
if (cluster.isMaster) {
	...
	try {
		...
		let cssserver = new nodestatic.Server(CONFIG_DIR);
		let avatarserver = new nodestatic.Server(CONFIG_DIR + 'avatars');
	...
	console.log('Servidor iniciado en: ' + (Config.bindaddress + ':' + Config.port).blue);

	if (appssl) {
		server.installHandlers(appssl, {});
		appssl.listen(Config.ssl.port, Config.bindaddress);
		console.log('Servidor iniciado en: protocolo SSH, mediante el puerto: ' + Config.ssl.port);
	}

	console.log('Entra al servidor mediante: ' + ('http://' + (Config.bindaddress === '0.0.0.0' ? 'localhost' : Config.bindaddress) + ':' + Config.port).blue);
