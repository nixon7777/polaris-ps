/*--
    APIs de Comandos basicos
        Este archivo maneja los datos de los comandos
        enviados desde las salas, pm, chatgroups,
        etc.

        Las funciones para la carga de los comandos
        basicos (basicCommands y baseCommands) se
        encuentran en la parte de abajo.

        Se recomienda no modificar las funciones
        y datos amenos que se sepan editar, para
        no dañar el funcionamiendo del servidor.

        De este archivo depende el servidor...
--*/
...
// Carga de comandos y Plugins
// ----------
// Chat.baseCommands - Contiene los comandos principales.
// Chat.commands - Es donde se alojan los comandos y plugins.
...
class CommandContext {
	...
	parse(message) {
		...
		if (typeof commandHandler === 'function') {
			...
			if (this.cmdToken) {
				// Mensaje error: Comando no existente
		...
		// Salida del mensaje (de emisor a receptor)
	...
	splitCommand(message = this.message, recursing) {
		...
		// Arreglo de comandos para desarrolladores
		...
		do {
			...
			if (typeof commandHandler === 'string') {
				// Si un persona tipea un comando no existente
	...
	checkFormat(room, user, message) {
		...
		// Remover caracteres extras y nulos
	...
	canBroadcast(suppressMessage) {
		if (!this.broadcasting && this.cmdToken === BROADCAST_TOKEN) {
			...
			// Reutilización de comando de difunsion
	...
	runBroadcast(suppressMessage) {
		if (this.broadcasting || this.cmdToken !== BROADCAST_TOKEN) {
			// Cuando el comando ya esta en difuncion o no se esta tratando de difundir.
			return true;
		}
	...
	canEmbedURI(uri, isRelative) {
		...
		// Advertencia de sitios fuera de PS
		...
		// URI desconocido, permitir HTTP seguro
		return 'http://' + uri;
	}
	canHTML(html) {
		html = ('' + (html || '')).trim();
		...
		while ((match = images.exec(html))) {
			...
			if (!/width=([0-9]+|"[0-9]+")/i.test(match[0]) || !/height=([0-9]+|"[0-9]+")/i.test(match[0])) {
				// 'width' y 'height' son importantes para evitar que imagenes
				// grandes se vean execivamente grande en pantallas pequeñas.
		...
		// Chequeo de codigo en busca de etiquetas faltantes
...
// Command Parser
// ----------
// Uso: Chat.parse(message, room, user, connection)
//
// Si es un comando, se ejecuta el commnad, si
// no se muestra directamente en la sala.
//
// Ejemplo: Chat.parse("/join lobby", room, user, connection)
// Hará que el usuario se una a la sala 'lobby'.
//
// message: Es el mensaje que el usuario quiere decir.
// room: La sala en donde el usuario tipea el mensaje.
// user: Usuario que envia el mensaje.
// connection: La conexion por la cual el usuario envio el mensaje.
...
// Carga de comandos y plugins
// ----------
// Usando 'yaml.js' se establecen los plugins
...
Chat.loadCommands = function () {
	...
	const Plugins = {};
	const pluginList = yamljs.load(path.join(__dirname, 'plugins/z-index.yaml'));
	Object.assign(commands, require('./plugins/info').commands);

	for (let i = 0, len = pluginList.length; i < len; i++) {
		let pluginPath = pluginList[i];
		if (Plugins[pluginPath]) continue;
		Object.assign(Chat.commands, require('./plugins/' + pluginPath).commands);
	}
};
...
Chat.toDurationString = function (number, options) {
	// Informacion en: https://github.com/tc39/ecma402/issues/47
