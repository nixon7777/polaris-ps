...
class BattleDex {
	...
	includeMods() {
		...
		let modList = fs.readdirSync(path.resolve(__dirname, 'tiers/mods'));
	...
	includeData() {
		...
		let basePath = this.isBase ? './tiers/data/' : './tiers/mods/' + this.currentMod + '/';
		...
		for (let dataType of DATA_TYPES) {
			...
			if (!BattleData || typeof BattleData !== 'object') throw new TypeError("Exported property `Battle" + dataType + "`from `" + './tiers/data/' + DATA_FILES[dataType] + "` must be an object except `null`.");
	...
	includeFormats() {
		...
		let BattleAliases = this.loadDataFile('./tiers/data/', 'Aliases');
		this.data.Aliases = BattleAliases;

		// Load formats
		let Formats;
		try {
			Formats = require('./tiers/formats').Formats;
		} catch (e) {
			if (e.code !== 'MODULE_NOT_FOUND') throw e;
		}
		if (!Array.isArray(Formats)) throw new TypeError("Exported property `Formats` from `" + "./tiers/formats.js" + "` must be an array.");
